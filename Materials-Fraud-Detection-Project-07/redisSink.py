...
def write_to_redis_mysql(df, epochId):
    r = redis.Redis(host='redis', db=0, port=6379, decode_responses=True)
    for row in df.collect():
        redis_key_mysql = f"Mysql:{row['mysql_customer']}"
        redis_value_mysql = {
            "purchase": row['mysql_purchase'],
            "clerk": row['mysql_clerk'],
            "store": row['mysql_store']
        }
        r.hset(redis_key_mysql, mapping=redis_value_mysql)  

#🚩 The decode_responses=True parameter in the Redis Python client (redis-py)
# is used to automatically 🛑decode responses from bytes to strings.🛑
# By default, Redis returns data as bytes (binary data),
# but setting decode_responses=True ensures that the data is 
# returned as strings, which is often more convenient to work with in Python.

def write_to_redis_postgres(df, epochId):
    r = redis.Redis(host='redis', db=1, port=6379, decode_responses=True)
    for row in df.collect():
        redis_key_postgres = f"Postgresql:{row['postgres_customer']}"
        redis_value_postgres = {
            "purchase": row['postgres_purchase'],
            "clerk": row['postgres_clerk'],
            "store": row['postgres_store']
        }
        r.hset(redis_key_postgres, mapping=redis_value_postgres)             


query_redis_mysql = parsed_stream_mysql.writeStream \
    .outputMode("append") \
    .foreachBatch(write_to_redis_mysql) \
    .trigger(processingTime='30 seconds') \
    .start()

query_redis_postgres= parsed_stream_postgres.writeStream \
    .outputMode("append") \
